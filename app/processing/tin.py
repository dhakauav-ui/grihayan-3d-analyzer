...
def generate_tin_surface(
    ...
    # Extract coordinates as contiguous float64 arrays with NaN protection
    x_vals = np.nan_to_num(working_df[x_col].to_numpy(dtype=np.float64), nan=0.0)
    y_vals = np.nan_to_num(working_df[y_col].to_numpy(dtype=np.float64), nan=0.0)
    z_vals = np.nan_to_num(working_df[rl_col].to_numpy(dtype=np.float64), nan=0.0)
    num_points = len(x_vals)

    # Calculate center for local coordinate centering in 3D WebGL (preventing float precision jitter)
    center_x = float(np.mean(x_vals)) if num_points > 0 else 0.0
    center_y = float(np.mean(y_vals)) if num_points > 0 else 0.0
    center_z = float(np.mean(z_vals)) if num_points > 0 else 0.0
    ...
    # Filter triangles by max edge length if specified
    ...
    if triangle_count > 0:
        ...
        total_2d_area = float(np.sum(np.nan_to_num(area_2d_triangles, nan=0.0)))
        ...
        total_3d_surface_area = float(np.sum(np.nan_to_num(area_3d_triangles, nan=0.0)))
    else:
        total_2d_area = 0.0
        total_3d_surface_area = 0.0

    # Calculate Convex Hull for Boundary Perimeter and Extrusion Skirt
    boundary_indices: List[int] = []
    hull_perimeter = float(2 * ((max_x - min_x) + (max_y - min_y)))
    hull_area = float(total_2d_area)

    try:
        hull = ConvexHull(pts_2d)
        boundary_indices = [int(i) for i in hull.vertices]
        hull_perimeter = float(hull.area)   # in 2D ConvexHull, .area is perimeter
        hull_area = float(hull.volume)      # in 2D ConvexHull, .volume is area
    except Exception:
        boundary_indices = []

    # Prepare local centered vertex array for WebGL (Float32)
    local_x = np.nan_to_num((x_vals - center_x).astype(np.float32), nan=0.0)
    local_y = np.nan_to_num((y_vals - center_y).astype(np.float32), nan=0.0)
    local_z = np.nan_to_num((z_vals - center_z).astype(np.float32), nan=0.0)
    ...
    return {
        ...
        "processing_time_sec": safe_float(round(elapsed_time, 3)),
        "center": {
            "x": safe_float(center_x),
            "y": safe_float(center_y),
            "z": safe_float(center_z)
        },
        "bounds": {
            "min_x": safe_float(min_x),
            "max_x": safe_float(max_x),
            "min_y": safe_float(min_y),
            "max_y": safe_float(max_y),
            "min_z": safe_float(min_z),
            "max_z": safe_float(max_z),
            "range_x": safe_float(max_x - min_x),
            "range_y": safe_float(max_y - min_y),
            "range_z": safe_float(max_z - min_z)
        },
        "geometry": {
        ...
        "metrics": {
            "area_2d_m2": safe_float(round(total_2d_area, 2)),
            "surface_area_3d_m2": safe_float(round(total_3d_surface_area, 2)),
            "hull_area_m2": safe_float(round(hull_area, 2)),
            "perimeter_m": safe_float(round(hull_perimeter, 2))
        },
